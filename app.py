...
def mobilenetv2_imagenet():
    ...
    uploaded_file = st.file_uploader("Choose an image...", type=["jpg", "png"])if uploaded_file is not None
...
def cifar10_classification():
    ...
    if uploaded_file is not None: # type: ignore
        image = Image.open(uploaded_file) # type: ignore
        st.image(image, caption='Uploaded Image', use_column_width=True)
...
def main():
    ...
    if choice == "MobileNetV2 (ImageNet)":
        ...
        cifar10_classification() # type: ignore
